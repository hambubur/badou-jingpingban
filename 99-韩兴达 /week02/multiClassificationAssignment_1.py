...
def predict(model_path, input_vec):
    ...
    for vec, res in zip(input_vec, result):
        # print("输入：%s, 预测类别：%d, 各类别得分：%s" % (vec, torch.argmax(res).item(), res))
        # 因为模型输出的是得分，所以需要用softmax函数转换成概率
        print("输入：%s, 预测类别：%d, 各类别概率：%s" % (vec, torch.argmax(res).item(), torch.softmax(res, dim=0)))
